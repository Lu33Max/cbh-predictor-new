...
const Content: React.FC = () => {
  ...
  const { data: entries } = api.google.getMany.useQuery({
    page: page,
    lines: pagelength,
  });

  const { data: count } = api.google.count.useQuery()

  return (
    <div className="max-h-[100vh] w-full overflow-y-scroll px-10 pb-6 pt-[12vh]">
      <Table entries={entries ?? []} type="Google" count={count ?? 0} pagelength={pagelength} page={page} setPagelength={setPagelength} setPage={setPage}/>
    </div>
  );
};
