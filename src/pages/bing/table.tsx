...
export default BingTable;

const Content: React.FC = () => {
  const [page, setPage] = useState(1);
  const [pagelength, setPagelength] = useState(50);

  const { data: entries } = api.bing.getMany.useQuery({
    page: page,
    lines: pagelength,
  });

  const { data: count } = api.bing.count.useQuery()

  return (
    <div className="max-h-[100vh] w-full overflow-y-scroll px-10 pb-6 pt-[12vh]">
      <Table entries={entries ?? []} type="Bing" count={count ?? 0} pagelength={pagelength} page={page} setPagelength={setPagelength} setPage={setPage}/>
    </div>
  );
};
