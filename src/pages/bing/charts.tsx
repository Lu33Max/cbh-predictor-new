...
import { type IBumpDatum, type ILineDatum, type IPieDatum } from "~/common/types";
...
//// MAPPING FUNCTIONS ////
function GetImpressions(entries: Bing[], minImpr: number, showOthers: boolean, dates: string[], period: number) {
...
function GetClicks(entries: Bing[], minClicks: number, showOthers: boolean, dates: string[], period: number) {
    const [data, setData] = useState<IPieDatum[]>([])
    let interval = period
...
function GetClicksAndImpressionsOverTime(dates: string[], period: number, allEntries: Bing[]) {
    ...
    useEffect(() => {
        ...
        if(currentDate !== undefined){
            setEntries(allEntries.filter(e => e.Date.localeCompare(currentDate) >= 0).sort((a,b) => a.Date < b.Date ? 1 : -1))
        }
...
function GetClickThroughOverTime(dates: string[], period: number, allEntries: Bing[]) {
    const [data, setData] = useState<ILineDatum[]>([])
    const [entries, setEntries] = useState(allEntries)
    let interval = period
    ...
    useEffect(() => {
        ...
        if(entries.length > 0 && newData[0]){
            ...
            for(let i = 0; i < clicksArr.length; i++){
                newData[0].data.push({
                    x: dates[i] ?? "None",
                    y: Math.round(((clicksArr[i] ?? 1) / (impressionsArr[i] ?? 1) + Number.EPSILON) * 1000) /10
                })
            }
            newData[0].data.reverse()
        }
...
function GetCustomAreaBump(terms: string[], dates: string[], period: number, allEntries: Bing[]) {
    const [data, setData] = useState<ILineDatum[]>([])
    const [entries, setEntries] = useState(allEntries)
    let interval = period
    ...
    useEffect(() => {
        const currentDate = dates[interval]
        if(currentDate !== undefined){
            setEntries(allEntries.filter(e => e.Date.localeCompare(currentDate) >= 0 && terms.find(t => t === e.Term) !== undefined).sort((a,b) => a.Date < b.Date ? 1 : -1))
        }
...
function GetCustomBar(terms: string[], dates: string[], period: number, allEntries: Bing[]): BarDatum[] {
...
const BingChart = () => {
    ...
    const onPeriodChange = (e: React.ChangeEvent<HTMLSelectElement>, index: number) => {
        const newPeriods = periods.map(period => { return period })
            ...
            if(includeExport[i] === true){
                ...
                if(element !== undefined && element !== null){
                    ...
                    if(i !== 4)
                    {
                        ...
                        if(count % 2 === 1)
                        {
                            pdf.text(headers[i] ?? "Default", pageWidth / 2, 15, { align: "center"})
                            pdf.addImage(data, 'PNG', marginX, 17, canvasWidth, canvasHeight)
                        }
                        else 
                        {
                            pdf.text(headers[i] ?? "Default", pageWidth / 2, (pageHeight / 2) + 10, { align: "center"})
                    ...
                    else
                    {
                        ...
                        pdf.text(headers[i] ?? "Default", pageWidth / 2, 60, { align: "center"})
    ...
    return(
        <div className={styles.body}>
            <button onClick={() => void router.push("/")} className={styles.button_backarrow}>&#60;</button>
            <div className={styles.export}>
                <button onClick={() => {setShowExport(false); void handleDownloadPdf()}} className={styles.button_export}>Export</button>
            ...
            <div className={styles.grid_container_3_items_small_mid}>
                <div className={styles.settings}>
                    ...
                    <select onChange={(e) => onPeriodChange(e, 0)}>
                        <option selected={true} value={0}>Last Month</option>
                ...
                <div className={styles.left_wrapper}>
                    <h3 className="h3">Impressions{showExport ? <input type="checkbox" checked={includeExport[0]} onChange={() => onIncludeChange(0)}></input> : <></>}</h3>
                    <div style={{width: "100%", height: "100%"}} ref={ref => !printRef.current.includes(ref) && printRef.current.push(ref)}>
                        <PieChart data={GetImpressions(allEntries ?? [], minImpr, showOthers, dates ?? [], periods[0] ?? 0)} scheme={primaryScheme}/>
                    </div>
                ...
                <div className={styles.middle_wrapper}>
                    <h3 className="h3">vs.</h3> <br/><br/><br/><br/>
                    <h4>Click-Through-Rate:</h4>
                    {GetClickThrough(allEntries ?? [], dates ?? [], periods[0] ?? 0)} %
                </div>
                <div className={styles.right_wrapper}>
                    <h3 className="h3">Clicks{showExport ? <input type="checkbox" checked={includeExport[1]} onChange={() => onIncludeChange(1)}></input> : <></>}</h3>
                    <div style={{width: "100%", height: "100%"}} ref={ref => !printRef.current.includes(ref) && printRef.current.push(ref)}>
                        <PieChart data={GetClicks(allEntries ?? [], minClicks, showOthers, dates ?? [], periods[0] ?? 0)} scheme={primaryScheme}/>
                    </div>
            ...
            <div className={styles.grid_container_2_items}>
                <div className={styles.settings}>
                    ...
                    <select onChange={(e) => onPeriodChange(e, 1)}>
                        <option value={2} selected={true}>Last 3 Months</option>
                        <option value={5}>Last 6 Months</option>
                        <option value={11}>Last Year</option>
                    </select>
                </div>
                <div className={styles.left_wrapper}>
                    <h3 className="h3">Impressions & Clicks{showExport ? <input type="checkbox" checked={includeExport[2]} onChange={() => onIncludeChange(2)}></input> : <></>}</h3>
                    <div style={{width: "100%", height: "100%"}} ref={ref => !printRef.current.includes(ref) && printRef.current.push(ref)}>
                        <LineChart data={GetClicksAndImpressionsOverTime(dates ?? [], periods[1] ?? 0, allEntries ?? [])} scheme={[primaryScheme[0] ?? "#000000", primaryScheme[8] ?? "#000000"]} axisBottom={"time"} axisLeft={""}/>
                    </div>
                </div>
                <div className={styles.middle_wrapper}>
                    <h3 className="h3">Click-Through-Rate{showExport ? <input type="checkbox" checked={includeExport[3]} onChange={() => onIncludeChange(3)}></input> : <></>}</h3>
                    <div style={{width: "100%", height: "100%"}} ref={ref => !printRef.current.includes(ref) && printRef.current.push(ref)}>
                        <LineChart data={GetClickThroughOverTime(dates ?? [], periods[1] ?? 0, allEntries ?? [])} scheme={[primaryScheme[10] ?? "#000000"]} axisBottom={"time"} axisLeft={"%"}/>
                    </div>
                </div>
            </div>
            {/* Third Block */}
            <div className={styles.grid_container_2_items_4_rows}>
                <div className={styles.settings}>
                    ...
                    <select onChange={(e) => onPeriodChange(e, 2)}>
                        <option value={2} selected={true}>Last 3 Month</option>
                        <option value={5}>Last 6 Months</option>
                        <option value={11}>Last Year</option>
                    </select>
                </div>
                <div className={styles.wrapper_2_wide_top}>
                    <h3 className="h3">Ranking over Time{showExport ? <input type="checkbox" checked={includeExport[4]} onChange={() => onIncludeChange(4)}></input> : <></>}</h3>
                    <div style={{width: "100%", height: "100%"}} ref={ref => !printRef.current.includes(ref) && printRef.current.push(ref)}>
                        <AreaBump data={GetCustomAreaBump(terms, dates ?? [], periods[2] ?? 0, allEntries ?? [])} scheme={tempScheme}/>
                    </div>
                </div>
                ...
                <div className={styles.wrapper_left_bottom}>
                    <h3 className="h3">Terms per Month{showExport ? <input type="checkbox" checked={includeExport[5]} onChange={() => onIncludeChange(5)}></input> : <></>}</h3>
                    <div style={{width: "100%", height: "85%"}} ref={ref => !printRef.current.includes(ref) && printRef.current.push(ref)}>
                        <BarChart data={GetCustomBar(terms, dates ?? [], periods[2] ?? 0, allEntries ?? [])} scheme={tempScheme} keys={terms} index={"date"} xAxis={"dates"} yAxis={""}/>
                    </div>
                </div>
                <div className={styles.wrapper_right_bottom}>
                    <h3 className="h3">Terms over Time{showExport ? <input type="checkbox" checked={includeExport[6]} onChange={() => onIncludeChange(6)}></input> : <></>}</h3>
                    <div style={{width: "100%", height: "85%"}} ref={ref => !printRef.current.includes(ref) && printRef.current.push(ref)}>
